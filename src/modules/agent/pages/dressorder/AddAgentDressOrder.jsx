...
import { useAgentDressOrder } from "../../agenthooks/useAgentDressOrder";
import useUnsavedChangesWarning from "../../../../common/hooks/useUnsavedChangesWarning";
import { getAgentProfileAPI } from "../../../../api/agent/agentDressOrderAPI";

const AddAgentDressOrder = () => {
  const navigate = useNavigate();
  const { createOrder, loading } = useAgentDressOrder();
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await getAgentProfileAPI();
        // Adjust based on actual API response structure
        const profile = response.data?.data || response.data || response;

        const fullName =
          `${profile.first_name || ""} ${profile.last_name || ""}`.trim();
        const code = profile.agent_code || profile.agent_id || "";
        ...
        const agent = userCookie ? JSON.parse(userCookie) : {};
        setAgentName(
          `${agent.first_name || ""} ${agent.last_name || ""}`.trim(),
        );
        setAgentCode(agent.agent_code || agent.agent_id || "");
        setAgentId(agent.agent_id || "");

        const fallbackData = {
          agent_id: agent.agent_id || "",
          dress_size: agent.dress_size || "", // use cookie's dress_size if available
          dress_quantity: 1,
          notes: "",
        };
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="mb-6">
        <button
          onClick={() => navigate("/agent/dress-orders")}
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Agent Info – disabled, auto-filled */}
          <div>
...
export default AddAgentDressOrder;
