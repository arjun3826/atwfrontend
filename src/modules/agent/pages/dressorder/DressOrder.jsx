...
import { Eye, Plus, Shirt, Package, Filter, X } from "lucide-react";
...
const AgentDressOrders = () => {
  ...
  const {
    ...
    setSelectedOrder,
  } = useAgentDressOrder({ autoFetch: true });

  const [filtersVisible, setFiltersVisible] = useState(false);
  const [isViewModalOpen, setIsViewModalOpen] = useState(false);

  // const openViewModal = async (order) => {
  //   if (order.dress_size && order.status) {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="p-6 w-full mx-auto min-h-[70vh]"
    >
      {/* Header */}
      <motion.div
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => navigate("/agent/dress-orders/add")}
...
export default AgentDressOrders;
