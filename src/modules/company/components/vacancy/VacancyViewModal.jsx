...
const VacancyViewModal = ({
  ...
  const getStatusConfig = () => {
    switch (vacancy.status?.toLowerCase()) {
      case "active":
        return {
          text: "Active",
  ...
  const getRateDisplay = () => {
    if (!vacancy.base_rate) return "N/A";
    if (vacancy.rate_type === "salary") {
      return `₹${vacancy.base_rate} / month`;
    } else {
      const rateTypeMap = {
        hourly: "hour",
        daily: "day",
      ...
      const unit = rateTypeMap[vacancy.rate_type] || "";
      return `₹${vacancy.base_rate} per ${unit}`;
    }
  };

  const getBreakDisplay = () => {
    if (vacancy.break_type === "paid") {
      return `Paid`;
    } else if (vacancy.break_type === "unpaid") {
      return `Unpaid (${vacancy.break_duration_minutes} min)`;
    } else {
  ...
  const getJobTypeDisplay = () => {
    if (vacancy.rate_type === "salary") return "Salary";
    const map = {
      hourly: "Per Hour",
      daily: "Daily",
      pcs: "Per Piece",
    ...
    return map[vacancy.rate_type] || vacancy.rate_type;
  };

  const schedules = vacancy.schedules || [];
    ...
    if (schedule.mode === "dates") {
      ...
      if (schedule.dates && Array.isArray(schedule.dates)) {
        if (
          ...
          // New format: array of date strings (ISO with time)
          datesList = schedule.dates.map((d) =>
            formatShortDate(d.split("T")[0]),
          );
        } else {
          // Old format: array of objects with date property
      ...
      // Extract weekdays regardless of format
      let weekdays = [];
      if (schedule.weekdays && Array.isArray(schedule.weekdays)) {
        if (
          ...
          // Old format: array of objects with weekday property
    ...
    if (schedule.mode === "salary") {
      return (
        <p className="text-sm text-gray-700 font-medium">
          Starting Date: {formatShortDate(schedule.start_date)}
        </p>
      );
    }
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center flex-wrap gap-2 mt-2">
                      <span
                        className={`inline-flex px-3 py-1 rounded-full text-xs font-medium ${statusConfig.className}`}
                      >
                        {statusConfig.text}
                      </span>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <StatCard
                  ...
                  value={vacancy.number_of_workers}
                />
                <StatCard
                  icon={<Clock className="text-green-500" />}
                  label="Shift"
                  value={`${vacancy.shift_start_time?.substring(0, 5) || "?"} - ${vacancy.shift_end_time?.substring(0, 5) || "?"}`}
                />
                <StatCard
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                >
                  ...
                  <DetailItem
                    label="Meals"
                    value={vacancy.meal_provided ? "Provided" : "Not Provided"}
                  />
                  <DetailItem
                    label="Notes"
                    value={vacancy.notes_to_workers || "None"}
                  />
                </Card>
                {/* Right Column */}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-3">
                  ...
                  {onEdit && (
                    <button
                      onClick={() => {
                        if (vacancy.status === "expired") return;
