...
import { useVacancyApplications } from "../../companyhooks/useVacancyApplications";
import {
  getEligibleWorkersAPI,
  assignWorkerAPI,
  getVacancyAttendanceAPI,
  updateAttendanceAPI,
  createFeedbackAPI, // <-- added
...
const ViewAppliedWorkers = () => {
  ...
  const {
    ...
    downloadDocument,
    refresh,
  } = useVacancyApplications(vacancyId);
  ...
  //     applications.length +
  //     applications.filter(
  //       (a) => a.onBoardingStatus === "joined" && a.terminationStatus === null,
  //     ).length +
  //     applications.filter((a) => a.terminationStatus !== null).length,
  ...
  //   notJoined: applications.filter(
  //     (a) =>
  //       a.status === "accepted" &&
  //       a.onBoardingStatus === null &&
  //       a.terminationStatus === null,
  //   ).length,

  //   terminated: applications.filter((a) => a.terminationStatus !== null).length,
  // };
  ...
  // ---------- Eligible Workers Modal Handlers ----------
  ...
  const handleAssignWorkers = async () => {
    ...
    try {
      ...
      refresh(); // reload applications list
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "Error",
  ...
  //   const from = formatDateForInput(firstDay);
  //   const to = formatDateForInput(lastDay);

  //   setFromDate(from);
  //   setToDate(to);

  //   fetchAttendanceLogs(workerId, from, to);
  // };
  const openAttendanceModal = (workerId = null, vacancyStartDate = null) => {
    ...
    // Use vacancy schedule start date if available
    const from = vacancyStartDate
      ? vacancyStartDate
      : formatDateForInput(new Date(today.getFullYear(), today.getMonth(), 1));

    const to = formatDateForInput(
  ...
  const openFeedbackModal = (workerId, workerName) => {
    setFeedbackData({
      workerId,
      vacancyId,
      workerName,
  ...
  //   try {
  //     const feedbackResponse = await createFeedbackAPI({
  ...
  //     // Check response body status, not HTTP status
  //     const responseStatus = feedbackResponse?.data?.status;
  //     const responseSuccess = feedbackResponse?.data?.success;

  //     // If API returns status 500 in response body
  //     if (responseStatus === 500 || responseSuccess === false) {
  //       Swal.fire({
  //         icon: "error",
  //         title: "Feedback Failed",
  //         text: feedbackResponse?.data?.message || "Failed to submit feedback.",
  //       });
  //       return; // STOP - Don't call termination API
  //     }

  //     // Feedback successful
  //     await terminateWorker(feedbackData.workerId);
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      ...
      if (
        feedbackResponse?.data?.status === 500 ||
        feedbackResponse?.data?.success === false
      ) {
        Swal.fire({
          icon: "error",
          title: "Feedback Failed",
      ...
      // Success only when both feedback and termination succeed
      Swal.fire({
        icon: "success",
        title: "Feedback Submitted & Worker Terminated",
    ...
    >
      ...
      >
        ...
        <StatCard
          ...
          value={counts?.applied_workers ?? 6}
        ...
        {filteredApps.length === 0 ? (
          ...
          filteredApps.map((app) => (
            <ApplicationCard
              ...
              updating={updating}
              onViewProfile={() => handleViewProfile(app.worker || app)}
              formatDate={formatDate}
              // onJoin={() => onboardWorker(app.worker.id)}
              // onLeft={() => openFeedbackModal(app.worker.id, app.worker.fullName)} // Changed
              // isJoining={isActionLoading(app.worker.id, "join")}
              // isLeaving={isActionLoading(app.worker.id, "left")}
      ...
      {/* Add Worker Modal - Eligible Workers */}
      {showEligibleModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          ...
          >
            ...
            <div className="p-4 border-t border-gray-200 flex justify-end gap-3">
              ...
              >
                <Plus size={16} />
                Add Selected ({selectedWorkers.length})
              </button>
            </div>
          </motion.div>
        </div>
...
export default ViewAppliedWorkers;
