...
const EditDesignation = () => {
  const { id: industryId, designation_id } = useParams();
  ...
  useEffect(() => {
    const fetchDesignation = async () => {
      try {
        setLoading(true);
        const response = await getDesignationByIdAPI(designation_id);
        // Assuming response structure: { data: { id, name, status, industry_id, ... } }
        const designation = response.data.data || response.data; // adjust as needed
        setFormData({
          name: designation.name,
          //   status: Number(designation.status), // ensure it's a number for radio buttons
        });
      } catch (error) {
    ...
    if (designation_id) fetchDesignation();
  }, [designation_id, industryId, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateDesignationAPI(designation_id, formData);
      Swal.fire({
        icon: "success",
        title: "Success!",
      ...
      <div className="flex-1 bg-gray-50 p-4 flex items-center justify-center">
        <div className="bg-white rounded-lg p-8 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">
            You don't have permission to edit designations.
          </p>
          <button
            onClick={() =>
              navigate(`/admin/industries/designations/${industryId}`)
  ...
  return (
    ...
    >
      <motion.div className="p-4 pb-0">
        <Breadcrumb
          items={[
            { label: "All Industries", path: "/admin/industries" },
            {
              label: industryName,
      ...
      <motion.div className="" variants={itemVariants}>
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Edit Designation</h1>
        </div>
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, status: 1 })}
              ...
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, status: 0 })}
                  className="w-4 h-4 text-blue-600"
                />
                <span>Inactive</span>
              </label>
