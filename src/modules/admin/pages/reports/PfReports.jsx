...
const PfReports = () => {
  ...
  if (!hasPermission("reports", "pf_reports")) {
    return (
      <div className="flex-1 bg-gray-50 p-4 flex items-center justify-center">
        <div className="bg-white rounded-lg p-8 text-center max-w-md shadow-sm">
          ...
          <p className="text-gray-600 mb-6">
            You don't have permission to view PF Reports.
          </p>
          <button
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await reportsApi.getPFReportPreview(params);
  ...
  const handleDownload = async (type) => {
    ...
    try {
      ...
      const response =
        type === "challan"
          ? await reportsApi.downloadPFReport(params)
          : await reportsApi.downloadECRReport(params);
      ...
      a.download =
        type === "challan"
          ? `PF_Report_${selectedYear}_${selectedMonth}.xlsx`
          : `PF_ECR_${selectedYear}_${selectedMonth}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast.success(
        `${type === "challan" ? "Excel" : "ECR Text"} download started`,
      );
    } catch (error) {
    ...
    >
      <motion.div variants={itemVariants} className="w-full max-w-7xl mx-auto">
        ...
        <div className="mb-6 flex justify-between items-end">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">PF Reports</h1>
            <p className="text-gray-500 text-sm">
              Generate monthly PF & ECR reports
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6 mb-6">
          <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
            ...
            Compliance Uploads (EPFO)
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { id: "EPFO_ECR", label: "EPFO ECR (PDF)" },
              { id: "EPFO_CHALLAN", label: "EPFO CHALLAN (PDF)" },
              { id: "EPFO_TRRN", label: "EPFO TRRN (PDF)" },
            ].map((docType) => {
                ...
                >
                  ...
                  {doc ? (
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <label
                        className={`cursor-pointer text-xs flex items-center gap-1.5 px-3 py-1.5 bg-white border border-blue-200 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors w-max font-semibold ${!isValid || uploading ? "opacity-50 pointer-events-none" : ""}`}
                      >
        ...
        {reportData.length > 0 && !loading && (
          ...
          >
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              <div>
                <h3 className="font-bold text-gray-800">PF Report Preview</h3>
              ...
              <div className="flex gap-3">
                ...
                <motion.button
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => handleDownload("ecr")}
                  disabled={downloading}
                  className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white rounded-xl font-semibold shadow-sm shadow-purple-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {downloading === "ecr" ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
              ...
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-gray-100 text-gray-700 uppercase text-[10px]">
                  <tr>
                    ...
                    <th
                      rowSpan="2"
                      className="px-3 py-3 border border-gray-200"
                    >
                      UAN
                    </th>
                    <th
                    ...
                    <th
                      colSpan="4"
                      className="px-3 py-2 border border-gray-200 text-center bg-blue-50"
                    >
                      Total Wages (Rs.)
                    </th>
                    <th
                      colSpan="4"
                      className="px-3 py-2 border border-gray-200 text-center bg-purple-50"
                    >
                  ...
                  <tr>
                    <th className="px-3 py-2 border border-gray-200 text-center bg-blue-50/50">
                      Gross
                    </th>
                    <th className="px-3 py-2 border border-gray-200 text-center bg-blue-50/50">
                      EPF
                    ...
                    <th className="px-3 py-2 border border-gray-200 text-center bg-purple-50/50">
                      EE PF
                    </th>
                    <th className="px-3 py-2 border border-gray-200 text-center bg-purple-50/50">
                      ER EPS
                ...
                <tbody className="divide-y divide-gray-200">
                  {reportData.map((row, index) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 border-x border-gray-100">
                        {row.name}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-right font-medium">
                        ₹{row.gross.toLocaleString()}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-right">
                        ₹{row.epf_wages.toLocaleString()}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-right">
                        ₹{row.eps_wages.toLocaleString()}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-right">
                        ₹{row.edli_wages.toLocaleString()}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-right text-blue-700 font-medium">
                        ₹{row.ee_pf.toLocaleString()}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-right text-purple-700">
                        ₹{row.er_eps.toLocaleString()}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-right text-purple-700 font-medium">
                        ₹{row.er_pf_diff.toLocaleString()}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-right text-gray-600">
                        ₹{row.admin_charges.toLocaleString()}
                      </td>
                      <td className="px-3 py-2 border-x border-gray-100 text-center">
                        {row.ncp_days}
                      </td>
                    </tr>
                  ))}
                ...
                {totals && (
                  <tfoot className="bg-gray-50 font-bold border-t-2 border-gray-300">
                    <tr>
                      ...
                      <td className="px-3 py-3 text-right">
                        ₹{totals.epf_wages.toLocaleString()}
                      </td>
                      <td className="px-3 py-3 text-right">
                        ₹{totals.eps_wages.toLocaleString()}
                      </td>
                      <td className="px-3 py-3 text-right">
                        ₹{totals.edli_wages.toLocaleString()}
                      </td>
                      <td className="px-3 py-3 text-right text-blue-700">
                        ₹{totals.ee_pf.toLocaleString()}
                      </td>
                      <td className="px-3 py-3 text-right text-purple-700">
                        ₹{totals.er_eps.toLocaleString()}
                      </td>
                      <td className="px-3 py-3 text-right text-purple-700">
                        ₹{totals.er_pf_diff.toLocaleString()}
                      </td>
                      <td className="px-3 py-3 text-right text-gray-600">
                        ₹{totals.admin_charges.toLocaleString()}
                      </td>
